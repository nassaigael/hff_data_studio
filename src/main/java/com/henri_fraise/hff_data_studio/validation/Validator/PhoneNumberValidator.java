...
public class PhoneNumberValidator implements ConstraintValidator<ValidPhoneNumber, String> {
	...
	public boolean isValid(String value, ConstraintValidatorContext context) {
		...
		if (!countryCode.isEmpty() && !cleaned.startsWith(countryCode)) {
			...
			context.buildConstraintViolationWithTemplate("Phone number must start with country code: " + countryCode).addConstraintViolation();
			return false;
		}

		if (!PHONE_PATTERN.matcher(cleaned).matches()) {
			context.disableDefaultConstraintViolation();
			context.buildConstraintViolationWithTemplate("Phone number format is invalid. Excepted format: [+][country code][number]").addConstraintViolation();
			return false;
		}
		return true;
	}
