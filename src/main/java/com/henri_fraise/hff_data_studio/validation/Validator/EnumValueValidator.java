...
public class EnumValueValidator implements ConstraintValidator<ValidEnumValue, String> {
	...
	public boolean isValid(String value, ConstraintValidatorContext context) {
		...
		for (Enum<?> enumConstant : enumClass.getEnumConstants()) {
			...
			if (ignoreCase) if (enumName.equalsIgnoreCase(trimmed)) return true;
			else if (enumName.equals(trimmed)) return true;
		}
		context.disableDefaultConstraintViolation();
		context.buildConstraintViolationWithTemplate("" + "Value must be one of: " + Arrays.toString(enumClass.getEnumConstants())).addConstraintViolation();
		return false;
	}
}
