...
public class ExportController {
  ...
  public ResponseEntity<ExportResponse> exportResults(@Valid @RequestBody ExportRequest request) {
    return ResponseEntity.status(HttpStatus.CREATED).body(exportService.exportResults(request));
  }
  ...
  public ResponseEntity<PageResponse<ExportResponse>> getExportsByExecution(
      @PathVariable UUID executionId, @PageableDefault(size = 20) Pageable pageable) {
