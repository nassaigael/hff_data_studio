...
public class CategoryService {
  ...
  public UserCategory getCategoryEntityByLabel(String label) {
    return categoryRepository
        .findByLabel(label)
        .orElseThrow(
            () -> new ResourceNotFoundException("Category not found with label: " + label));
  }
  ...
  public List<UserCategoryResponse> getCategoriesByAccessLevel(int accessLevel) {
    try {
      List<UserCategory> categories =
          categoryRepository.findByAccessLevelGreaterThanEqual(accessLevel);
      return categories.stream().map(categoryMapper::toResponse).collect(Collectors.toList());
    } catch (Exception ex) {
  ...
  public UserCategoryResponse createCategory(CategoryCreationRequest request) {
    if (categoryRepository.existsByLabel(request.getLabel())) {
      throw new ResourceAlreadyExistsException(
          "Category already exists with label: " + request.getLabel());
    }
  ...
  public UserCategory createCategoryEntity(UserCategory category) {
    try {
      if (categoryRepository.existsByLabel(category.getLabel())) {
        throw new ResourceAlreadyExistsException(
            "Category already exists with label: " + category.getLabel());
      }
      UserCategory saved = categoryRepository.save(category);
    ...
    try {
      ...
      if (userCount > 0) {
        throw new IllegalStateException(
            "Cannot delete category with " + userCount + " users assigned");
      }
  ...
  public void updateCategoryPermissions(UUID categoryId, List<UUID> permissionIds) {
    ...
    try {
      ...
      log.info(
          "Permissions updated for category: {} ({} permissions)",
          category.getLabel(),
          permissions.size());
      ...
      auditLogService.logAction(
          ...
          "UserCategory",
          categoryId,
          "Permissions updated for category "
              + category.getLabel()
              + " - "
              + permissions.size()
              + " permissions");
  ...
  public UserCategory getDefaultCategory() {
    return categoryRepository
        .findByLabel("INVITE")
        .orElseGet(() -> categoryRepository.findFirstByOrderByAccessLevelAsc().orElse(null));
  }
