...
public class AuditLogService {
  ...
  @Transactional
  public void logActionWithUserAndIp(
      String action, String entity, UUID entityId, String details, UUID userId, String ipAddress) {
    try {
      User user = userService.getUserEntityById(userId);
      AuditLog auditLog =
          AuditLog.builder()
  ...
  public Page<AuditLogResponse> getAuditLogs(
    ...
    try {
      ...
      if (userId != null && startDate != null && endDate != null && action != null) {
        logs =
            auditLogRepository.findByUserIdAndActionAndActionDateBetween(
                userId, action, startDate, endDate, pageable);
      } else if (userId != null && startDate != null && endDate != null) {
        logs =
            auditLogRepository.findByUserIdAndActionDateBetween(
        ...
        logs =
            auditLogRepository.findByActionAndActionDateBetween(
                action, startDate, endDate, pageable);
      } else if (action != null) {
  ...
  public Page<AuditLogResponse> getAuditLogsByDateRange(
    ...
    try {
      Page<AuditLog> logs =
          auditLogRepository.findByActionDateBetween(startDate, endDate, pageable);
      return logs.map(auditLogMapper::toResponse);
    } catch (Exception ex) {
  ...
  public Page<AuditLogResponse> getAuditLogsByEntity(
      String entity, UUID entityId, Pageable pageable) {
    try {
      Page<AuditLog> logs =
          auditLogRepository.findByConcernedEntityAndEntityId(entity, entityId, pageable);
      return logs.map(auditLogMapper::toResponse);
    } catch (Exception ex) {
    ...
    try {
      AuditLog log =
          auditLogRepository
              .findById(logId)
              .orElseThrow(
                  () -> new ResourceNotFoundException("Audit log not found with id: " + logId));
  ...
  public Page<AuditLogResponse> searchAuditLogs(String searchTerm, Pageable pageable) {
    try {
      Page<AuditLog> logs =
          auditLogRepository.findByActionContainingIgnoreCase(searchTerm, pageable);
      return logs.map(auditLogMapper::toResponse);
    } catch (Exception ex) {
