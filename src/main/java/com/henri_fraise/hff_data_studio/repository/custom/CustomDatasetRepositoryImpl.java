...
public class CustomDatasetRepositoryImpl implements CustomDatasetRepository {
	...
	public Map<String, Object> getDatasetStatisticsMap() {
		...
		try {
			...
			Query projectQuery = entityManager.createNativeQuery(
					"SELECT COUNT(DISTINCT sf.project_id) FROM dataset d JOIN source_file sf ON d.source_file_id = sf.file_id"
			);
			stats.put("distinctProjects", getLongValue(projectQuery.getSingleResult()));

			Query fileQuery = entityManager.createNativeQuery(
					"SELECT COUNT(DISTINCT source_file_id) FROM dataset"
			);
	...
	public long countDatasetsCreatedAfter(LocalDateTime date) {
		try {
			Query query = entityManager.createQuery(
					"SELECT COUNT(d) FROM Dataset d WHERE d.createdAt > :date"
			);
	...
	public void deleteDatasetsByFileId(UUID fileId) {
		try {
			Query query = entityManager.createQuery(
					"DELETE FROM Dataset d WHERE d.sourceFile.id = :fileId"
			);
