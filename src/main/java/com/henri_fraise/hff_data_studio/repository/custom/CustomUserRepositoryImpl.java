...
""";
  ...
  public long countUsersCreatedBetween(LocalDateTime startDate, LocalDateTime endDate) {
    try {
      String sql =
          "SELECT COUNT(u.user_id) FROM users u WHERE u.created_at BETWEEN :startDate AND :endDate";
      Query query = entityManager.createNativeQuery(sql);
      query.setParameter("startDate", startDate);
      query.setParameter("endDate", endDate);
      Long result = (Long) query.getSingleResult();
  ...
  public void updateUserActivity(UUID userId, String activity) {
    try {
      String sql =
          "UPDATE users SET last_activity = NOW(), activity_log = CONCAT(activity_log, :activity)"
...
""";
  ...
  public long countUsersWithNoLogin() {
    try {
      String sql =
          "SELECT COUNT(u.user_id) FROM users u WHERE u.last_login IS NULL AND u.is_active = true";
      Query query = entityManager.createNativeQuery(sql);
      Long result = (Long) query.getSingleResult();
      return result != null ? result : 0L;
