...
public class CustomProjectRepositoryImpl implements CustomProjectRepository {
  ...
  public ProjectStatisticsResponse getProjectStatistics() {
    Query query =
        entityManager.createNativeQuery(
            "SELECT COUNT(*) as total_projects, SUM(CASE WHEN status = 'IN_PROGRESS' THEN 1 ELSE 0"
                + " END) as in_progress, SUM(CASE WHEN status = 'COMPLETED' THEN 1 ELSE 0 END) as"
                + " completed, SUM(CASE WHEN status = 'ARCHIVED' THEN 1 ELSE 0 END) as archived,"
                + " (SELECT COUNT(DISTINCT creator_user_id) FROM project) as total_creators,"
                + " (SELECT COUNT(*) FROM project WHERE created_at >= NOW() - INTERVAL '30 days')"
                + " as new_projects_last_30_days FROM project");
  ...
  public long countActiveProjectsByUser(UUID userId) {
    Query query =
        entityManager.createQuery(
            "SELECT COUNT(p) FROM Project p WHERE p.creator.id = :userId AND p.status !="
                + " 'ARCHIVED'");
  ...
  public void archiveProjectsByUser(UUID userId) {
    Query query =
        entityManager.createQuery(
            "UPDATE Project p SET p.status = 'ARCHIVED' " + "WHERE p.creator.id = : user_id");
