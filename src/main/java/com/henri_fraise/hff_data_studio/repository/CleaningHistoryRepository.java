...
public interface CleaningHistoryRepository extends JpaRepository<CleaningHistory, UUID> {
	...
	@Query("SELECT h FROM CleaningHistory h WHERE h.dataset.id = :datasetId AND h.executedAt >= :since")
	...
	@Query("SELECT h.dataset.id, COUNT(h) FROM CleaningHistory h GROUP BY h.dataset.id ORDER BY COUNT(h) DESC")
	List<Object[]> countByDatasetGrouped();

	@Query("SELECT h.user.id, COUNT(h) FROM CleaningHistory h GROUP BY h.user.id ORDER BY COUNT(h) DESC")
	List<Object[]> countByUserGrouped();

	// ===== GET LATEST =====
	@Query("SELECT h FROM CleaningHistory h WHERE h.dataset.id = :datasetId ORDER BY h.executedAt DESC")
	List<CleaningHistory> findLatestByDatasetId(@Param("datasetId") UUID datasetId, Pageable pageable);

	@Query("SELECT h FROM CleaningHistory h WHERE h.status = :status ORDER BY h.executedAt DESC")
	List<CleaningHistory> findLatestByStatus(@Param("status") CleaningHistoryStatus status, Pageable pageable);
