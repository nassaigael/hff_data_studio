...
public interface CleaningRuleRepository extends JpaRepository<CleaningRule, UUID> {
  ...
  void updateRuleStatus(@Param("rule_id") UUID ruleId, @Param("is_active") Boolean isActive);

  @Modifying
  @Transactional
  @Query("UPDATE CleaningRule r SET r.executionOrder = :order WHERE r.id = :rule_id")
  void updateExecutionOrder(@Param("rule_id") UUID ruleId, @Param("order") Integer order);
  ...
  @Query(
      "SELECT r FROM  CleaningRule  r WHERE r.column.dataset.id = :dataset_id AND r.isActive = true"
          + " ORDER BY r.executionOrder ASC ")
  List<CleaningRule> findActiveRulesByDatasetIdOrdered(@Param("dataset_id") UUID datasetId);

  @Query(
      "SELECT r FROM CleaningRule r WHERE r.column.dataset.id = :dataset_id AND r.executionOrder ="
          + " :order")
  List<CleaningRule> findByDatasetIdAndExecutionOrder(
      @Param("dataset_id") UUID datasetId, @Param("order") Integer order);

  @Query(
      "SELECT MAX(r.executionOrder) FROM CleaningRule  r WHERE r.column.dataset.id = :dataset_id")
  ...
  List<CleaningRule> findByColumnIdAndRuleType(
      @Param("column_id") UUID columnId, @Param("rule_type") RuleType ruleType);

  @Query(
      "SELECT r FROM CleaningRule  r WHERE  r.column.dataset.id = :dataset_id AND r.ruleType ="
          + " :rule_type")
  List<CleaningRule> findByDatasetIdAndRuleType(
      @Param("dataset_id") UUID datasetId, @Param("rule_type") RuleType ruleType);
}
