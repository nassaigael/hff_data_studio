...
public interface ChartRepository extends JpaRepository<Chart, UUID> {
  ...
  @Query(
      "SELECT c FROM Chart c WHERE c.chartType = :chartType AND c.result.execution.id ="
          + " :executionId")
  List<Chart> findByChartTypeAndExecutionId(
      @Param("chartType") ChartType chartType, @Param("executionId") UUID executionId);
  ...
  Optional<Chart> findByResultIdAndChartType(
      @Param("resultId") UUID resultId, @Param("chartType") ChartType chartType);
