...
public interface AuditLogRepository extends JpaRepository<AuditLog, UUID> {
	...
	Page<AuditLog> findByConcernedEntityAndEntityId(String concernedEntity, UUID entityId, Pageable pageable);

	Page<AuditLog> findByActionAndActionDateBetween(String action, LocalDateTime startDate, LocalDateTime endDate, Pageable pageable);

	Page<AuditLog> findByUserIdAndActionDateBetween(UUID userId, LocalDateTime startDate, LocalDateTime endDate, Pageable pageable);

	Page<AuditLog> findByUserIdAndAction(UUID userId, String action, Pageable pageable);

	Page<AuditLog> findByUserIdAndActionAndActionDateBetween(UUID userId, String action, LocalDateTime startDate, LocalDateTime endDate, Pageable pageable);
	...
	Page<AuditLog> findByActionDateBetween(@Param("startDate") LocalDateTime startDate,
	                                       @Param("endDate") LocalDateTime endDate,
	                                       Pageable pageable);

	@Query("SELECT a FROM AuditLog a WHERE a.user.id = :userId AND a.actionDate BETWEEN :startDate AND :endDate")
	...
	List<AuditLog> findByConcernedEntityAndEntityId(@Param("entity") String entity,
	...
	@Query("SELECT a.concernedEntity, COUNT(a) FROM AuditLog a GROUP BY a.concernedEntity ORDER BY COUNT(a) DESC")
	...
	@Query("SELECT DATE(a.actionDate) as date, COUNT(a) FROM AuditLog a WHERE a.actionDate BETWEEN :startDate AND :endDate GROUP BY DATE(a.actionDate) ORDER BY date")
	List<Object[]> countGroupByDate(@Param("startDate") LocalDateTime startDate,
	                                @Param("endDate") LocalDateTime endDate);

	@Query("SELECT YEAR(a.actionDate) as year, MONTH(a.actionDate) as month, COUNT(a) FROM AuditLog a GROUP BY YEAR(a.actionDate), MONTH(a.actionDate) ORDER BY year DESC, month DESC")
	...
	@Query("SELECT a FROM AuditLog a WHERE a.user.id = :userId AND a.action = :action ORDER BY a.actionDate DESC")
