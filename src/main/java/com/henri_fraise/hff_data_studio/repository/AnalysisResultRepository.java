...
public interface AnalysisResultRepository extends JpaRepository<AnalysisResult, UUID> {
  ...
  List<AnalysisResult> findByExecutionIdAndResultTypeIn(
      UUID executionId, List<ResultType> resultTypes);
  ...
  @Query(
      "SELECT r FROM AnalysisResult r WHERE r.execution.id = :execution_id AND r.resultType ="
          + " 'CHART'")
  ...
  @Query(
      "SELECT r FROM AnalysisResult r WHERE r.fileFormat = :file_format AND r.execution.id ="
          + " :execution_id")
  List<AnalysisResult> findByExecutionIdAndFileFormat(
      @Param("execution_id") UUID executionId, @Param("file_format") String fileFormat);
}
