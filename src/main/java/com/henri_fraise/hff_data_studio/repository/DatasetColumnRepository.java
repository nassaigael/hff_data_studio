...
public interface DatasetColumnRepository extends JpaRepository<DatasetColumn, UUID> {
	...
	@Modifying
	@Transactional
	@Query("UPDATE DatasetColumn c SET c.nullCount = :null_count, c.uniqueCount = :unique_count WHERE c.id = :column_id")
	void updateStats(
			@Param("column_id") UUID columnId,
			@Param("null_count") Integer nullCount,
			@Param("unique_count") Integer uniqueCount);

	@Query("SELECT c FROM DatasetColumn  c WHERE  c.dataset_id = :dataset_id AND " +
	...
	@Query("SELECT c FROM DatasetColumn c WHERE c.dataset_id = :dataset_id AND c.uniqueCount = c.row_count")
	...
	@Query("SELECT c FROM DatasetColumn c WHERE c.dataset_id = :dataset_id AND c.detectedType = :detected_type AND c.targetType IS NULL ")
	List<DatasetColumn> findColumnsWithDetectedTypeNotConverted(@Param("dataset_id") UUID datasetId, @Param("detected_type") ColumnType detectedType);
