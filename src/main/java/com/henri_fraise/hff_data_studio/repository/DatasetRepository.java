...
public interface DatasetRepository  extends JpaRepository<Dataset, UUID> {
	...
	void updateIsCleaned(
			@Param("dataset_id") UUID datasetId, @Param("is_cleaned") Boolean isCleaned);

	@Modifying
	@Transactional
	@Query("UPDATE Dataset d SET d.rowCount = :row_count, d.columnCount = :column_count WHERE d.id = :dataset_id")
	void updateStats(
			@Param("dataset_id") UUID datasetId, @Param("row_count") Integer rowCount, @Param("column_count") Integer columnCount);

	@Query("SELECT d FROM Dataset d WHERE  d.project_id = :project_id AND " +
			"(:search_term IS NULL OR LOWER(d.datasetName) LIKE LOWER(CONCAT('%', :search_term, '%') ) )")
	Page<Dataset> searchProjectDatasets(@Param("project_id") UUID projectId,
			@Param("search_term") String searchTerm, Pageable pageable);
	...
	long countDatasetsCreatedBetween(@Param("start_date") String startDate, @Param("end_date") String endDate);
