...
public interface UserCategoryRepository extends JpaRepository<UserCategory, UUID> {
  ...
  @Query(
      "SELECT c.label, COUNT(u) FROM UserCategory c LEFT JOIN c.users u GROUP BY c.label ORDER BY"
          + " COUNT(u) DESC")
  List<Object[]> getCategoryStatistics();

  @Query(
      "SELECT c FROM UserCategory c WHERE c.label IN ('ADMIN', 'DATA_ANALYST', 'CONSULTANT',"
          + " 'INVITE')")
