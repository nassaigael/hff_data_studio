...
public interface ExplorationReportRepository extends JpaRepository<ExplorationReport, UUID> {
  ...
  void updateQualityScore(@Param("report_id") UUID reportId, @Param("score") BigDecimal score);

  @Modifying
  @Transactional
  @Query("UPDATE ExplorationReport r SET r.reportPdfPath = :pdf_path WHERE r.id = :report_id")
  void updateReportPdfPath(@Param("report_id") UUID reportId, @Param("pdf_path") String pdfPath);
