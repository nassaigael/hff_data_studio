...
public interface ExplorationReportRepository extends JpaRepository<ExplorationReport, UUID> {
  ...
  Page<ExplorationReport> findByQualityScoreBetween(
  ...
  void updateQualityScoreByDatasetId(
      @Param("datasetId") UUID datasetId, @Param("score") BigDecimal score);
  ...
  @Transactional
  @Query(
      "UPDATE ExplorationReport r SET r.reportPdfPath = :pdfPath WHERE r.dataset.id = :datasetId")
  void updateReportPdfPathByDatasetId(
      @Param("datasetId") UUID datasetId, @Param("pdfPath") String pdfPath);
  ...
  List<ExplorationReport> findByGeneratedAtBetween(
      @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

  @Query("SELECT r FROM ExplorationReport r WHERE r.generatedAt BETWEEN :startDate AND :endDate")
  Page<ExplorationReport> findByGeneratedAtBetween(
  ...
  @Query(
      "SELECT COUNT(r) FROM ExplorationReport r WHERE r.generatedAt BETWEEN :startDate AND"
          + " :endDate")
  ...
  @Query(
      "SELECT r.qualityScore, COUNT(r) FROM ExplorationReport r GROUP BY r.qualityScore ORDER BY"
          + " r.qualityScore")
  ...
  @Query(
      "SELECT r FROM ExplorationReport r JOIN FETCH r.dataset WHERE r.qualityScore >= :minScore"
          + " ORDER BY r.qualityScore DESC")
  List<ExplorationReport> findHighQualityReportsWithDataset(
  ...
  @Query(
      "SELECT r.id, r.dataset.id, r.dataset.datasetName, r.qualityScore, r.generatedAt "
  ...
  void updateQualityScoreBulk(
      @Param("reportIds") List<UUID> reportIds, @Param("score") BigDecimal score);

  @Modifying
  @Transactional
  @Query("UPDATE ExplorationReport r SET r.qualityScore = :score WHERE r.dataset.id IN :datasetIds")
  void updateQualityScoreByDatasetIds(
      @Param("datasetIds") List<UUID> datasetIds, @Param("score") BigDecimal score);
