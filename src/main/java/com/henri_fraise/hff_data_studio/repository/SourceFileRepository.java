...
public interface SourceFileRepository extends JpaRepository<SourceFile, UUID> {
  ...
  void updateProcessingStatusByProjectId(@Param("projectId") UUID projectId, @Param("status") FileProcessingStatus status);
  ...
  List<SourceFile> findByProjectIdAndUploadedAtBetween(@Param("projectId") UUID projectId,
                                                       @Param("startDate") LocalDateTime startDate,
                                                       @Param("endDate") LocalDateTime endDate);

  @Query("SELECT COUNT(f) FROM SourceFile f WHERE f.uploadedAt BETWEEN :startDate AND :endDate")
  long countByUploadedAtBetween(@Param("startDate") LocalDateTime startDate,
                                @Param("endDate") LocalDateTime endDate);
  ...
  List<SourceFile> findRecentFilesByProjectId(@Param("projectId") UUID projectId, Pageable pageable);
  ...
  List<SourceFile> findOldFilesByStatus(@Param("status") FileProcessingStatus status,
  ...
  @Query("SELECT CASE WHEN COUNT(f) > 0 THEN true ELSE false END FROM SourceFile f WHERE f.fileName = :fileName AND f.project.id = :projectId AND f.id != :fileId")
  boolean existsByFileNameAndProjectIdAndNotId(@Param("fileName") String fileName,
                                               @Param("projectId") UUID projectId,
                                               @Param("fileId") UUID fileId);

  @Query("SELECT CASE WHEN COUNT(f) > 0 THEN true ELSE false END FROM SourceFile f WHERE f.project.id = :projectId AND f.processingStatus = :status")
  boolean existsByProjectIdAndStatus(@Param("projectId") UUID projectId,
                                     @Param("status") FileProcessingStatus status);

  @Query("SELECT MIN(f.sizeBytes), MAX(f.sizeBytes), AVG(f.sizeBytes), COUNT(f) FROM SourceFile f WHERE f.project.id = :projectId")
  Object[] getFileSizeStatisticsByProjectId(@Param("projectId") UUID projectId);

  @Query("SELECT DATE(f.uploadedAt), COUNT(f) FROM SourceFile f GROUP BY DATE(f.uploadedAt) ORDER BY DATE(f.uploadedAt) DESC")
