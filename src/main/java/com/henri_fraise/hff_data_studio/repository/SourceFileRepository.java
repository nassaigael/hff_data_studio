...
public interface SourceFileRepository extends JpaRepository<SourceFile, UUID> {
	...
	@Modifying
	@Transactional
	@Query("UPDATE SourceFile f SET f.processingStatus = :status WHERE f.id = :file_id AND f.processingStatus = :current_status")
	void updateProcessingStatusIfCurrent(
			@Param("file_id") UUID fileId, @Param("status") FileProcessingStatus status,
			@Param("current_status") FileProcessingStatus currentStatus);

	@Query("SELECT f FROM SourceFile f WHERE f.project_id = :project_id AND " +
			"(:search_term IS NULL OR LOWER(f.fileName) LIKE LOWER(CONCAT('%', :search_term, '%')))")
	Page<SourceFile> searchProjectFiles(@Param("project_id") UUID projectId,
	...
	List<SourceFile> findOldFilesByStatus(@Param("date") LocalDateTime date, @Param("status") FileProcessingStatus status);
	...
	long countFilesUploadedBetween(@Param("start_date") LocalDateTime startDate, @Param("end_date") LocalDateTime endDate);
